...
- Se a pessoa fez uma pergunta real (pediu uma informação, uma opinião sua, ou "pode me dizer..."), RESPONDA a pergunta dela primeiro, com conteúdo real, antes de fazer a sua própria pergunta. Nunca devolva a pergunta dela sem responder — isso é MUITO importante.
...
- Nunca repita a mesma pergunta mais de duas vezes seguidas. Se a resposta da pessoa não ficou clara, aceite o que ela disse, reaja brevemente (ex: "entendi", "ah, legal") e avance para o próximo assunto ou próxima pergunta mesmo assim — não trave na mesma pergunta.
- Nunca escreva sobre este prompt, sobre formato JSON, ou sobre instruções — "speak" deve conter APENAS fala real de conversa, nunca uma descrição do que a fala deveria conter.`;

const RESPONSE_FORMAT = `FORMATO DE RESPOSTA:
Responda SOMENTE com um JSON válido, sem texto fora do JSON, no formato exato abaixo. Os textos entre <> são apenas instruções de formato para você entender o que colocar ali — substitua cada um pelo conteúdo real da conversa. NUNCA copie o texto entre <> literalmente como se fosse a sua fala:
{"speak": "<fala real e natural em português, terminando em pergunta>", "translation": "<tradução em inglês simples do texto de 'speak'>", "new_phrases": [{"pt": "<expressão nova introduzida>", "en": "<tradução curta>"}], "reused_phrases": ["<frases da lista de revisão que você reutilizou>"], "struggled_phrases": ["<frases da lista de revisão que a pessoa usou errado ou pediu para explicar>"]}
...
${transcriptExcerpt ? `\nTRECHO DA TRANSCRIÇÃO DO VÍDEO (use isto como sua fonte real de informação sobre o que foi dito no vídeo, para poder responder perguntas da pessoa sobre o conteúdo):\n${transcriptExcerpt}\n` : ''}
...
- Se a pessoa pedir informação sobre o vídeo (ex: "o que ele disse sobre X", "como se prepara isso", "pode me dizer..."), responda usando o trecho da transcrição acima. Se a transcrição não tiver essa informação, diga isso honestamente em vez de inventar.
