...
export function saveVideoLesson({ url, title, summary_pt, conversation_context, questions, transcript }) {
  const result = db.prepare(`
    INSERT INTO videos (url, title, summary_pt, conversation_context, questions, transcript) VALUES (?, ?, ?, ?, ?, ?)
  `).run(url, title || '', summary_pt || '', conversation_context || '', JSON.stringify(questions || []), transcript || '');
  return Number(result.lastInsertRowid);
}
